...
struct editorSyntax {
    char *filetype;
    // filematch is an array of strings, each string 
    ...
    // flags is a bit field containing flags
    int flags;
};
...
typedef struct erow {
    ...
    char *chars;
    char *render;
    // highlight, maps indexes of render to categories
    unsigned char *hl;  
    int hl_open_comment; // useful to keep track of multi-line comments
} erow;
...
struct editorConfig {
    ...
    int numrows;
    erow *row; // editor row
    int dirty;  // 0 if file is not modified
    char *filename;
    char statusmsg[80];
...
void editorUpdateSyntax(erow *row) {
    ...
    int prev_sep = 1;
    int in_string = 0;   // will store ' or ", 0 if not in string
    ...
    while (i < row->rsize) {
        ...
        if (E.syntax->flags & HL_HIGHLIGHT_STRINGS) {
            if (in_string) {
                row->hl[i] = HL_STRING;
                // Manage escape quotes by skipping
                if (c == '\\' && i + 1 < row->size) {
        ...
        if (E.syntax->flags && HL_HIGHLIGHT_NUMBERS) {
            if ((isdigit(c) && (prev_sep || prev_hl == HL_NUMBER)) ||
               (c == '.' && prev_hl == HL_NUMBER)) {  // support digits with periods
                row->hl[i] = HL_NUMBER;
                i++;
                prev_sep = 0;
                continue;
...
char *editorRowsToString(int *buflen) {
    ...
    *buflen = totlen;
    ...
    char *p = buf; // p will move forward in *buf
    for (j = 0; j < E.numrows; j++) {
    ...
    return buf;  // caller should free() the memory
}
...
void editorFindCallback(char *query, int key) {
    ...
    for (i = 0; i < E.numrows; i++) {
        current += direction;  // loop around baby
...
void editorOpen(char *filename) {
    ...
    free(line);
    fclose(fp);
    E.dirty = 0;  // we just appended rows, but file is clean
}
...
void editorDrawRows(struct abuf *ab) {
    ...
    for (y = 0; y < E.screenrows; y++) {
        ...
        if (filerow >= E.numrows) {
            ...
            int j;
            for (j = 0; j < len; j++) {
                // Print weird characters in normalish way
                if (iscntrl(c[j])) {
                    char sym = (c[j] <= 26) ? '@' + c[j] : '?';
            ...
            abAppend(ab, "\x1b[39m", 5); // reset to normal, white color
        }
...
void editorDrawStatusBar(struct abuf *ab) {
    ...
    // Show if the file has been modified (dirty)
    int len = snprintf(status, sizeof(status), "%.20s - %d lines %s",
            E.filename ? E.filename : "[No Name]", E.numrows,
            E.dirty ? "(modified)" : "");
    int rlen = snprintf(rstatus, sizeof(rstatus), "%s | %d/%d",
            E.syntax ? E.syntax ->filetype: "no ft", E.cy + 1, E.numrows);
    if (len > E.screencols) len = E.screencols; // cap it
...
void initEditor() {
    ...
    E.screenrows -= 2; // accomodate space for the status bar, and status message
}
